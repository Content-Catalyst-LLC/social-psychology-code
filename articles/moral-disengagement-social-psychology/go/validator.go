...
var requiredColumns = []string{
	...
	"harmful_decision", "policy_endorsement", "unethical_intention", "response_time_ms",
}

func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run go/validator.go data/moral_disengagement_trials.csv")
	}
	...
	for rowNum, row := range rows[1:] {
		...
		for _, col := range requiredColumns[5:] {
			if col == "harmful_decision" {
				checkRange(row, index, col, 0, 1, line, &errors)
			} else if col == "response_time_ms" {
				checkRange(row, index, col, 150, 1000000, line, &errors)
			} else {
