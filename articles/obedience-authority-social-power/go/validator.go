...
var requiredColumns = []string{
	...
	"condition", "trial", "authority_legitimacy", "authority_proximity", "institutional_prestige",
	"command_clarity", "cost_of_defiance", "escalation_step", "responsibility_displacement",
	"moral_conflict", "victim_proximity", "harm_salience", "peer_dissent", "peer_compliance",
	"role_identification", "mission_identification", "obeyed", "resisted", "hesitation",
	"protest", "response_time_ms", "perceived_responsibility_after",
}

func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run go/validator.go data/obedience_trials.csv")
	}
	...
	for rowNum, row := range rows[1:] {
		...
		checkRange(row, index, "escalation_step", 1, 1000000, line, &errors)
		checkRange(row, index, "response_time_ms", 150, 10000000, line, &errors)

		for _, col := range []string{"obeyed", "resisted", "protest"} {
			checkRange(row, index, col, 0, 1, line, &errors)
		}

		for _, col := range []string{
			"authority_legitimacy", "authority_proximity", "institutional_prestige",
			"command_clarity", "cost_of_defiance", "responsibility_displacement",
			"moral_conflict", "victim_proximity", "harm_salience", "peer_dissent",
			"peer_compliance", "role_identification", "mission_identification",
			"hesitation", "perceived_responsibility_after",
		} {
