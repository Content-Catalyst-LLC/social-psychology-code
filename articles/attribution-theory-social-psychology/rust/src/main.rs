...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: cargo run --manifest-path rust/Cargo.toml -- data/attribution_trials.csv");
        std::process::exit(1);
    }
    ...
    let target_i = idx("target_type");
    let internal_i = idx("attribution_internal");
    let external_i = idx("attribution_external");
    let responsibility_i = idx("responsibility_rating");
    let blame_i = idx("blame_rating");
    let help_i = idx("help_support");
    ...
    for line in lines {
        ...
        let key = format!("{}:{}", fields[condition_i], fields[target_i]);
        ...
        entry.help_sum += parse_f64(fields[help_i]);
    }

    println!("ConditionTarget,Trials,MeanInternal,MeanExternal,MeanResponsibility,MeanBlame,MeanHelpSupport");
