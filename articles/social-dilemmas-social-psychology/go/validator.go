...
var requiredColumns = []string{
	...
	"endowment", "contribution", "extraction", "mpcr", "trust_score", "norm_salience",
	"enforcement_signal", "fairness_score", "institutional_legitimacy", "monitoring_strength",
	"sanction_probability", "sanction_severity", "reciprocity_expectation",
	"resource_stock", "group_welfare", "individual_payoff", "response_time_ms",
}

func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: go run go/validator.go data/social_dilemmas_trials.csv")
	}
	...
	for rowNum, row := range rows[1:] {
		...
		checkRange(row, index, "endowment", 0, 1000000, line, &errors)
		checkRange(row, index, "contribution", 0, 1000000, line, &errors)
		checkRange(row, index, "extraction", 0, 1000000, line, &errors)
		checkRange(row, index, "mpcr", 0, 1000000, line, &errors)
		...
		for _, col := range []string{
			"trust_score", "norm_salience", "enforcement_signal", "fairness_score",
			"institutional_legitimacy", "monitoring_strength", "sanction_severity",
